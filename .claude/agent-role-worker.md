...
* **Never wait or poll**: no `sleep` to wait, no `while`/`until` polling loops, no re-encodings of the same thing via `seq`/`pgrep`/`tail -f`, no polling harness internals (task `.output` files, subagent transcripts, the process table). `sleep` as rate-limit backoff between API calls is fine — the harness rejects a bare foreground `sleep`, so write that backoff as `python3 -c 'import time; time.sleep(150)'`, and use it for nothing else.
